...
UniValue getinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  \"balance\": xxxxxxx,         (numeric) the total vitalcoin balance "
        "of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nReturn information about the given vitalcoin address.\n"
        "\nArguments:\n"
...
UniValue createmultisig(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 2) {
    std::string msg =
        ...
        "vitalcoin addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"key\"    (string) vitalcoin address or hex-encoded public "
        "key\n"
        "       ,...\n"
        "     ]\n"
...
UniValue echo(const JSONRPCRequest &request) {
  if (request.fHelp)
    throw std::runtime_error(
        ...
        "vitalcoin-cli and the GUI. There is no server-side difference.");
