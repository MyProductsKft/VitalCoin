 ...
 * between vitalcoind, and vitalcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("vitalcoin-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", VITALCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", VITALCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/vitalcoin/vitalcoin>";
    const std::string URL_WEBSITE = "<https://vitalcoincore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("vitalcoin-loadblk");
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-disablefork", false)) {
        printf("NOTE: disabling fork; this node will run on the vitalcoin network\n");
        FORK_ALLOWED = false;
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", VITALCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if vitalcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if vitalcoin is started while in a temporary directory.\n",
    ...
    GenerateVitalcoins(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
