...
void static VitalcoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in VitalcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);

            // LogPrintf("Running VitalcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateVitalcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VitalcoinMiner, boost::cref(chainparams)));
}
