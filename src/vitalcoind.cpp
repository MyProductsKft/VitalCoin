...
bool AppInit(int argc, char *argv[]) {
  ...
  // Parameters
  //
  // If Qt is used, parameters/vitalcoin.conf are parsed in qt/vitalcoin.cpp's
  // main()
    ...
    for (int i = 1; i < argc; i++) {
      if (!IsSwitchChar(argv[i][0])) {
        fprintf(stderr, "Error: Command line contains unexpected token '%s', "
                        "see vitalcoind -h for a list of options.\n",
                argv[i]);
        exit(EXIT_FAILURE);
      }
    }

    // -server defaults to true for vitalcoind but not for the GUI so do this
    // here
