...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Vitalcoin address (e.g. %1)").arg(QString::fromStdString(DummyAddress(Params()))));
...
bool parseVitalcoinURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    for (QList<QPair<QString, QString>>::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!VitalcoinUnits::parse(VitalcoinUnits::VTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatVitalcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(VitalcoinUnits::format(VitalcoinUnits::VTC, info.amount, false, VitalcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    boost::filesystem::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VITALCOIN_CONF_FILENAME));
    ...
    /* Open vitalcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Vitalcoin (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("vitalcoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a vitalcoin.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the vitalcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef vitalcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (vitalcoinAppUrl == nullptr) {
        return false;
    }

    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, vitalcoinAppUrl);

    CFRelease(vitalcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add vitalcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, vitalcoinAppUrl, nullptr, nullptr);
