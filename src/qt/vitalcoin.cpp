...
{
    return QCoreApplication::translate("vitalcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. vitalcoin_de.qm (shortcut "de" needs to be defined in vitalcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. vitalcoin_de_DE.qm (shortcut "de_DE" needs to be defined in vitalcoin.qrc)
...
class VitalcoinCore : public QObject
{
    Q_OBJECT
public:
    explicit VitalcoinCore(interfaces::Node& node);
...
class VitalcoinApplication : public QApplication
{
    Q_OBJECT
public:
    explicit VitalcoinApplication(interfaces::Node& node, int& argc, char** argv);
...
#include <qt/vitalcoin.moc>

VitalcoinCore::VitalcoinCore(interfaces::Node& node) : QObject(), m_node(node)
{
}
...
VitalcoinApplication::VitalcoinApplication(interfaces::Node& node, int& argc, char** argv) : QApplication(argc, argv),
                                                                                             coreThread(0),
                                                                                             m_node(node),
                                                                                             optionsModel(0),
                                                                                             clientModel(0),
...
void VitalcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the VitalcoinApplication constructor, or after it, because
...
{
    window = new VitalcoinGUI(m_node, platformStyle, networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, SIGNAL(timeout()), window, SLOT(detectShutdown()));
}

void VitalcoinApplication::createSplashScreen(const NetworkStyle* networkStyle)
{
...
void VitalcoinApplication::startThread()
{
    ...
    VitalcoinCore* executor = new VitalcoinCore(m_node);
...
void VitalcoinApplication::addWallet(WalletModel* walletModel)
{
#ifdef ENABLE_WALLET
...
void VitalcoinApplication::removeWallet()
{
#ifdef ENABLE_WALLET
...
void VitalcoinApplication::initializeResult(bool success)
{
    ...
    if (success) {
        ...
        // vitalcoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", VitalcoinGUI::tr("A fatal error occurred. Vitalcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", VitalcoinGUI::DEFAULT_UIPLATFORM), true, OptionsCategory::GUI);
}

#ifndef VITALCOIN_QT_TEST
int main(int argc, char* argv[])
{
    ...
    VitalcoinApplication app(*node, argc, argv);
    ...
    /// 6. Determine availability of data and blocks directory and parse vitalcoin.conf
    ...
    // vitalcoin: links repeatedly have their payment requests routed to this process:
