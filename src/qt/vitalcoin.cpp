...
static void initTranslations(QTranslator &qtTranslatorBase,
  ...
  // Load e.g. vitalcoin_de.qm (shortcut "de" needs to be defined in
  // vitalcoin.qrc)
  if (translatorBase.load(lang, ":/translations/"))
    QApplication::installTranslator(&translatorBase);

  // Load e.g. vitalcoin_de_DE.qm (shortcut "de_DE" needs to be defined in
  // vitalcoin.qrc)
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
  ...
  /// 6. Determine availability of data directory and parse vitalcoin.conf
  ...
  try {
    gArgs.ReadConfigFile(gArgs.GetArg("-conf", VITALCOIN_CONF_FILENAME));
  } catch (const std::exception &e) {
  ...
  // vitalcoin: links repeatedly have their payment requests routed to this
  // process:
