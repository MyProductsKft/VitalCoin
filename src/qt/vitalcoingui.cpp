...
#include <qt/vitalcoingui.h>

#include <qt/vitalcoinunits.h>
...
VitalcoinGUI::VitalcoinGUI(interfaces::Node& node, const PlatformStyle* _platformStyle, const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                                                                                             m_node(node),
...
void VitalcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and vitalcoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Vitalcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Vitalcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a vitalcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Vitalcoin command-line options").arg(tr(PACKAGE_NAME)));
...
#ifdef ENABLE_WALLET
bool VitalcoinGUI::addWallet(WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool VitalcoinGUI::removeWallet(WalletModel* walletModel)
{
...
bool VitalcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
...
void VitalcoinGUI::showHelpMessageClicked()
{
...
void VitalcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Vitalcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void VitalcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}
...
void VitalcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay) {
        if (header)
...
void VitalcoinGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
#ifdef ENABLE_WALLET
void VitalcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
...
void VitalcoinGUI::updateWalletStatus()
{
    if (!walletFrame) {
        return;
    }
...
void VitalcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void VitalcoinGUI::toggleHidden()
{
...
static bool ThreadSafeMessageBox(VitalcoinGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
                                                                                               ...
                                                                                               menu(0)
{
    ...
    for (const VitalcoinUnits::Unit unit : units) {
        max_width = qMax(max_width, fm.width(VitalcoinUnits::longName(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    for (VitalcoinUnits::Unit u : VitalcoinUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(VitalcoinUnits::longName(u)), this);
...
{
    setText(VitalcoinUnits::longName(newUnits));
}
