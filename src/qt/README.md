This directory contains the VitalcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Vitalcoin UI.
...
ClientModel is used by the main application `vitalcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/vitalcoin/vitalcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a vitalcoin:... link from a browser).
...
* UI elements like VitalcoinAmountField, which inherit from QWidget.
* `vitalcoinstrings.cpp`: automatically generated
* `vitalcoinunits.(h/cpp)`: VTC / mVTC / etc handling
* `callback.h`
...
* don't change `local/vitalcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "vitalcoin-qt" under "Run", which is where you can also add command line arguments)
