...
The RPM spec file provided here is for VitalCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://vitalcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[VitalCoin git repository](https://github.com/vitalcoin/vitalcoin) but are not part of
...
Currently only `Source10` is used, for the example `vitalcoin.conf` file.
...
in the vitalcoin git repository. At present only an SVG version of the VitalCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the vitalcoin
...
needed when 0.12.1 is released, a proper fix is already in the VitalCoin git
master branch.
...
idea to build VitalCoin Core against a static version of that library compiled
...
The `vitalcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `vitalcoin-server` package. At this point, most RPM based
...
The `vitalcoin-server` package should have SELinux support. How to properly do
...
VitalCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/vitalcoin/). He has been
packaging VitalCoin for Fedora at least since 2012.
...
4. I renamed the `vitalcoin` package that contains the Qt GUI to `vitalcoin-core` as
that appears to be how the general population refers to it, in contrast to
`vitalcoin-xt` or `vitalcoin-classic`. I wanted to make sure the general population
