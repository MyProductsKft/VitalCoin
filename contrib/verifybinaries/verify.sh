...
###   vitalcoincore.org and vitalcoin.org and compares them.
...
WORKINGDIR="/tmp/vitalcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vitalcoincore.org"
HOST2="https://vitalcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="vitalcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vitalcoin-',
...
if [ $? -ne 0 ]; then
   echo "vitalcoin.org failed to provide signature file, but vitalcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vitalcoin.org and vitalcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Vitalcoin Core binary release signing key installed?"
   fi
