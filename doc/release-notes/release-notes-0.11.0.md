Vitalcoin Core version 0.11.0 is now available from:

  <https://vitalcoin.org/bin/vitalcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer VTC/kB fee to be
...
For example, add the following to `vitalcoin.conf`:
...
related to the blockchain in the vitalcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Vitalcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Vitalcoin Core will 
...
For an example script see [vitalcoin-submittx](https://github.com/laanwj/vitalcoin-submittx).
...
- #5334 `cf87536` libvitalcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new vitalcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only VitalcoinAmountField
- #5753 `0f44672` Add vitalcoin logo to about screen
...
- #5809 `46bfbe7` Add vitalcoin-cli man page
...
- #5849 `41259ca` contrib/init/vitalcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Vitalcoin-Qt.app to Vitalcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for vitalcoin-tx)
