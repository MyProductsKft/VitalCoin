...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-vitalcoin-host/README.md).
...
Clone the git repositories for vitalcoin and Gitian.
...
in the vitalcoin repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vitalcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/vitalcoin/vitalcoin
...
For example:
```bash
URL=https://github.com/laanwj/vitalcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit vitalcoin=${COMMIT} --url vitalcoin=${URL} ../vitalcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vitalcoin=${COMMIT} --url vitalcoin=${URL} ../vitalcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vitalcoin=${COMMIT} --url vitalcoin=${URL} ../vitalcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the vitalcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../vitalcoin/contrib/gitian-descriptors/*|sort|uniq )
...
VTCPATH=/some/root/path/vitalcoin
SIGPATH=/some/root/path/vitalcoin-detached-sigs

./bin/gbuild --url vitalcoin=${VTCPATH},signature=${SIGPATH} ../vitalcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vitalcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vitalcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vitalcoin-osx-build.assert
```
...
[vitalcoin-core/gitian.sigs](https://github.com/vitalcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
